        ...
        if line.len() >= 12 {
            ...
            if let Some('[') = chars.next() {
                if let Some(']') = chars.nth(10) {
                    ...
                    Err(ParserError(String::from("Failed to parse gtest: Missing closing bracket")))
                }
            } else {
                Err(ParserError(String::from("Failed to parse gtest: Missing opening bracket")))
            }
        } else {
            Err(ParserError(String::from("Failed to parse gtest: Message is too short")))
        }
    }
}
